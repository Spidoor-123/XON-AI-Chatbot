...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const chatHTML = `
    <div id="xon-chat-container" style="position:fixed; bottom:20px; right:20px; z-index:2147483647; font-family:'Poppins', sans-serif;">
      <button id="xon-chat-btn" style="background: linear-gradient(135deg, #061e59ff, #1d387fff); color:#fff; border:none; border-radius:50%; width:60px; height:60px; font-size:24px; cursor:pointer; box-shadow:0 4px 15px rgba(63, 113, 243, 0.4); transition: all 0.3s ease; display: flex; align-items: center; justify-content: center; outline: none;">
      ...
      <div id="xon-chat-window" style="display:none; width:380px; height:500px; background:#fff; border-radius:15px; box-shadow:0 10px 30px rgba(0,0,0,0.2); flex-direction:column; margin-bottom:15px; overflow:hidden; border: 1px solid rgba(63, 113, 243, 0.1);">
        <div style="background: linear-gradient(135deg, #031f65ff, #5b8aff); color:#fff; padding:18px; font-weight:600; display:flex; justify-content:space-between; align-items:center;">
          <span>XON AI Security Assistant</span>
          <button id="xon-chat-close" style="background:none; border:none; color:#fff; cursor:pointer; font-size:18px; outline: none;">&times;</button>
        </div>
        <div id="xon-chat-history" style="flex:1; padding:15px; overflow-y:auto; font-size:14px; background:#fdfdfd; display:flex; flex-direction:column; gap:10px;">
          <div style="background:#fff; border: 1px solid #eef2ff; padding:12px; border-radius:12px 12px 12px 0; max-width:85%; align-self:flex-start; color:#333; line-height:1.4; box-shadow: 0 2px 5px rgba(63,113,243,0.05);">
        ...
        <div style="padding:15px; border-top:1px solid #eee; background:#fff; display:flex; gap:10px; align-items:center;">
          ...
          <button id="xon-chat-send" style="background: linear-gradient(135deg, #092160ff, #0c2b7aff); color:#fff; border:none; width:40px; height:40px; border-radius:50%; cursor:pointer; display:flex; align-items:center; justify-content:center; box-shadow:0 2px 8px rgba(63, 113, 243, 0.3); outline: none;">
  ...
  const sendMessage = async () => {
    ...
    chatHistory.innerHTML += `
      <div style="background: #e8f0fe; padding:12px; border-radius:12px 12px 0 12px; max-width:85%; align-self:flex-end; color:#0056b3; line-height:1.4;">
          <b>You:</b> ${userMessage}
      </div>
    `;
